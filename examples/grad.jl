...
size = 10
m = [[0.0; 0.0] [0.0; size] [size; size] [size; 0.0]]
...
boundary_set = gennodeset(n->n.coords[2]>(size - 0.0000001), "boundary", geomesh.nodes)
#boundary_set2 = gennodeset(n->n.coords[2]<0.0001, "boundary2", geomesh.nodes)
append!(boundary_set, gennodeset(n->n.coords[1]>(size - 0.0000001), "boundary", geomesh.nodes))
...
fp = FEM.create_feproblem_grad("grad_jll_20", geomesh, [ele_section], [mat_section], bcs)
...
solver = NRSolver(abs_tol = 1e-2, max_iters = 20, alt_tol = 1e-6)
